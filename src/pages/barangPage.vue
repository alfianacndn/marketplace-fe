<template>
  <div class="pa-15">
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                v-bind="attrs"
                v-on="on"
                @click="title='Tambah Barang'"
              >
                Tambah Barang
              </v-btn>
            </template>

            <AddEdit :dialog.sync="dialogAdd" :title="title"/>
          </v-dialog>
        </div>
      </template>
      <download-excel
            ...
            <template v-slot:default>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="text-left">{{item.harga}}</td>
                  <td class="text-left">{{item.supplier.namaSupplier}}</td>
                  <td class="text-left">{{item.supplier.alamat}}</td>
                  <td class="text-left">{{item.supplier.noTelp}}</td>
                  <td class="text-left">
                    <v-icon color="red" class="mr-8" @click="forDelete(item)"> mdi-delete </v-icon>
                    <v-icon  color="green" @click="forEdit(item)"> mdi-pencil </v-icon>
                  </td>
                </tr>
              </tbody>
            </template>
    ...
    <v-dialog
      ...
      width="500"
    >
      <AddEdit :dialog.sync="dialogEdit" :title="title" :valueBarang="value"/>
    </v-dialog>
  </div>
</template>
...
export default {
  ...
  data(){
    return {
      items:[{namaBarang:'',stok:0,harga:'',supplier:{namaSupplier:'',alamat:'',noTelp:''}}],
      title:'',
      dialogAdd: false,
      dialogEdit:false,
      ...
      json_fields: {
          ...
          'No Telp Supplier' : 'supplier.noTelp'
        },
        json_meta: [
          [
            {
  ...
  methods:{
    ...
    forDelete(val){
      this.$http.delete("/barang/delete/" + val.id,{
        headers:{
    ...
    getData(){
      ...
      this.$http.get("/barang/find-all?limit="+this.currentTake+'&offset=' +this.currentPage,{
        headers:{
          Authorization: 'Bearer ' + localStorage.getItem('token')
        }
      })
